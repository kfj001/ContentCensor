...
(function (global) {
  ...
  function toPatterns(rows) {
    ...
    /**
    * Serialize a UI state for storage (UI §4.1 save): persist every DEFINED rule
    * (non-empty find) — INCLUDING disabled ones, whose enabled flag is kept — plus
    * a timestamp. Keeping disabled rows lets the popup report "active vs defined"
    * after a save; only blank / empty-find no-op rows are dropped. The caller wraps
    * this as a SINGLE atomic chrome.storage.sync.set — there is NO
    * chrome.storage.sync.clear() anywhere (M1 / MV3 Phase 0.2), which removes the
    * empty-window data race. (The content-script transform still filters to
    * enabled+non-empty via toPatterns, so a disabled row never matches.)
    *
   ...
   function serializeSync(state) {
      ...
      for (var i = 0; i < all.length; i++) {
        ...
        // Persist every DEFINED rule (non-empty find) WITH its enabled flag, so
        // the popup's "active vs defined" count survives a save. Only blank /
        // empty-find no-op rows are dropped.
        if (r.find) rows.push(r);
        }
      return {
