...
(function (global) {
  ...
  /**
   * Serialize a UI state for storage (UI §4.1 save): persist only enabled,
   * non-empty-find rules plus a timestamp. The caller wraps this as a SINGLE
   * atomic chrome.storage.sync.set — there is NO chrome.storage.sync.clear()
   * anywhere (M1 / MV3 Phase 0.2), which removes the empty-window data race.
   *
   * @param {object} state - { enabled, rows }
   * @returns {{contentCensorData: Array<object>, enabled: boolean, updatedAt: number}}
   */
